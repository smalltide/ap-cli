...
def aws(ctx, profile):
    ...
    if result.ok:
        click.secho(
            f'Add AP AWS Profile and Credential Successful: ',
            nl=False,
            fg='green',
            bold=True)
        ...
        click.secho(
            f'Add AP AWS Profile and Credential Failure', fg='red', bold=True)
...
def github(ctx, account, repository):
    ...
    is_linked = is_text_in_file('github', github_config) and is_text_in_file(
        account, github_config)
    ...
    if is_linked:
        ...
        if result.ok:
            click.secho(
                f'AP Link GitHub Repository Successful', fg='green', bold=True)
        else:
            click.secho(
                f'AP Link GitHub Repository Failure', fg='red', bold=True)
